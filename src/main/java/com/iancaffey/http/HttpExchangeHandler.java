...
import com.iancaffey.http.util.Response;
...
public class HttpExchangeHandler implements ExchangeHandler {
    ...
    public void accept(Exchange exchange) throws IOException {
        ...
        try {
            ...
            Response.code(ResponseCode.BAD_REQUEST).accept(reader, writer);
        } catch (RoutingException e) {
            Response.code(ResponseCode.NOT_FOUND).accept(reader, writer);
        } catch (Exception e) {
            e.printStackTrace();
            Response.code(ResponseCode.INTERNAL_SERVER_ERROR).accept(reader, writer);
        }
    }
}
